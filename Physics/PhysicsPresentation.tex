...
\begin{tikzpicture}
    ...
    \draw[thick] (0,0) circle (1.5cm); % Circle path
    \draw[fill=black] (0,0) circle (2pt); % Center point
    \draw[-stealth, thick] (0,0) -- node[midway, above] {\(\omega_0\)} (1.06,1.06); % Initial velocity
    \draw[-stealth, thick] (0,0) -- node[midway, below] {\(\omega\)} (-1.5,0); % Final velocity
    \draw[->, thick] (1.8,0) arc (0:340:1.8cm); % Angular path
    \node at (2.2,-1.7) {\(\theta\)};
\end{tikzpicture}
